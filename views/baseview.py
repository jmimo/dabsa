        ...
        if request:
            if request.cookies:
                ...
                if selected_file_id:
                    ...
                    if session:
                        session['selected_file'] = airspaceFile
        return {'files': AirspaceFile.query.all(), 'selected_file': airspaceFile, 'navloc': self.get_navloc()}

    def dispatch_request(self):
        if request.args.get('airspacefileid'):
            session['selected_file'] = AirspaceFile.query.get(request.args.get('airspacefileid'))

        if session:
            if session['selected_file']:
                response = make_response(self.render_template(self.get_objects()))
                response.set_cookie('selected_file', str(session['selected_file'].get_id()))
                return response
        else:
