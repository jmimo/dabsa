...
def igcparse(filename,filepointer,date):
    initial_line = filepointer.readline()
    if initial_line == None or initial_line[:4] != 'AFLY':
        raise SyntaxError('Invalid IGC File detected')
    ...
    try :
        ...
        raise SyntaxError('Error while parsing IGC File')
